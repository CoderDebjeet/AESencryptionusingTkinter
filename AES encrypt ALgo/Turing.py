...
class Win1:
    ...
    def __init__(self, master):
        ...
        self.img = ImageTk.PhotoImage(Image.open("crypt.jpeg").resize((580, 260)))
...
class Win2:
    def encrypts(self):
        ...
        self.t2.insert("1.0", ciphertext.hex())
...
class Win3:
    ...
    def decrypts(self):
        ciphertext = bytes.fromhex(self.t1.get("1.0", tk.END))
        pass_phrase = self.tkey.get().encode()  # Generate a 16-byte secret key
        nonce = ciphertext[:16]  # Extract the nonce from the ciphertext
        tag = ciphertext[16:]  # Extract the tag from the ciphertext
        cipher = AES.new(secret_key, AES.MODE_EAX, nonce=nonce)
        decrypted_message = cipher.decrypt(ciphertext)

        self.t2.delete('1.0', tk.END)
        self.t2.insert("1.0", decrypted_message.decode(errors='replace'))
